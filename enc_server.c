...
void handleClient(int connectionSocket) {
    ...
    numBytes = recv(connectionSocket, plaintext, sizeof(plaintext) - 1, 0);
    if (numBytes <= 0) {
    ...
    int plaintextLength;
    numBytes = receiveAll(connectionSocket, (char*)&plaintextLength, sizeof(plaintextLength));
    if (numBytes != sizeof(plaintextLength)) {
        close(connectionSocket);
        exit(1);
    }
    plaintextLength = ntohl(plaintextLength);
    if (plaintextLength > MAX_BUFFER - 1) {
        close(connectionSocket);
        exit(1);
    }
    memset(plaintext, 0, sizeof(plaintext));
    numBytes = receiveAll(connectionSocket, plaintext, plaintextLength);
    if (numBytes != plaintextLength) {
        close(connectionSocket);
        exit(1);
    }
    ...
    for (int i = 0; i < plaintextLength; i++) {
        int plainVal = charToValue(plaintext[i]);
        int keyVal = charToValue(key[i]);
        if (plainVal == -1 || keyVal == -1) {
        ...
        ciphertext[i] = valueToChar(cipherVal);
    }
    ...
    sendAll(connectionSocket, ciphertext, ciphertextLength);
